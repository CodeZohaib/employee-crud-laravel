   ...
   public function viewEmployeeData(string $id)
    ...
    {       
        if($request->hasFile('profile_picture'))
        {
            ...
            $file->move(public_path('profile_pictures'), $filename); // move directly to public/profile_pictures

            $path = 'profile_pictures/'.$filename;
        }
        else
        {
            $path = $employee->profile_pic_path ?? 'profile_pictures/profile.JPG';
        }
        ...
        if(!empty($id))
        {
            ...
            if($id && is_numeric($id))
            {
                ...
                if($employee)
                {
                   if($request->hasFile('profile_picture'))
                    {
                        if($employee->profile_pic_path != 'profile_pictures/profile.JPG' && file_exists(public_path($employee->profile_pic_path)))
                        {
                            unlink(public_path($employee->profile_pic_path)); // delete old image
                        }
        ...
        if(!empty($id))
        {
            ...
            if ($empId && is_numeric($empId)) 
            {
                ...
                if($employee)
                {
                  ...
                  if ($employee->delete()) 
                  {
                      if($employeePicPath != 'profile_pictures/profile.jpg' && file_exists(public_path($employeePicPath)))
                      {
                          unlink(public_path($employeePicPath)); // direct public folder se delete
                      }
    ...
    public function destroyMultiRecord(Request $request)
    {
        ...
        if ($deleteIDDecrypt) {
           ...
           foreach ($employees as $emp) {
            $employeePicPath = $emp->profile_pic_path;

            // Default image delete nahi hogi
            if ($employeePicPath != 'profile_pictures/profile.jpg' && file_exists(public_path($employeePicPath))) {
                unlink(public_path($employeePicPath));
            }
        }
